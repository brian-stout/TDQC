...
def comparenumberslie(guess, randomNumber):
    """This function does the same thing as comparenumbers() but lies about the
       results

       Additionally it returns a string stating which number it lied about so
       it can be called later.  This function return both objects in a list
       instead of a tuple to cut back on code lines and to make it clearer to
       read.  The reason is because a tuple can't unpack directly from
       a function return
       """

    if guess == randomNumber:
        return "You guessed right when I was about to lie!"
    elif guess > randomNumber:
        print(guess, "is too low  - please guess again", end="")
    ...
    while guess != randomNumber:
        try:
            ...
            if (shouldlie(.25) and haveLied == False):
                whenILied = comparenumberslie(guess, randomNumber)
                haveLied = True
            else:
